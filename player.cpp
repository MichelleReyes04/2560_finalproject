...
Player::Player(string playerName, int bet, int startingBalance) {
    ...
    this->stats.setTotalMoney(startingBalance);
}
...
void Player::callingHit(Deck& deck) {
    vector<string> card = deck.drawCards(1);
...
//function to output current bankroll balance
...
//function to access player's stats
//function to access player's stats
Stats& Player::getStats() {
    return stats;
}

//function to access player's stats without changing them
...
//function to set bankroll amount
void Player::setBankroll(int amount) {
    bankroll = amount;
    stats.setTotalMoney(amount);
}

//function to clear player hand before a new round
